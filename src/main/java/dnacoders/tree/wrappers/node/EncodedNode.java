...
public class EncodedNode<S extends DNAPointer> extends IDNAReference.DNAReference<S> {
    ...
    public static <S extends DNAPointer> EncodedNode<S> of(boolean isLeaf, IDNASketcher<S> sketcher, Function<Integer, Integer> tolerance, BaseSequence[] payloads) {
    ...
    public int oligosCount() {
        return oligos.length;
    }

    public long totalNucleotides() {
        return Arrays.stream(oligos).mapToLong(BaseSequence::length).sum();
    }
